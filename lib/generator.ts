...
export function generateStructuredPage(input: GenerationInput): GeneratedPage {
  ...
  const html = `<section class="generated-hero"><p class="generated-kicker">${escapeHtml(input.category)} · ${escapeHtml(input.pageType)}</p><h1>${escapeHtml(headline)}</h1><p>${escapeHtml(input.description)}</p><a class="generated-cta" href="${escapeHtml(input.affiliateUrl)}" rel="sponsored nofollow noopener" target="_blank">Explore ${safeName} ↗</a></section><aside class="generated-disclosure"><strong>Affiliate disclosure:</strong> ${disclosure}</aside><section><h2>What is ${safeName}?</h2><p>${escapeHtml(input.description)} This review focuses on practical value for ${safeAudience}.</p></section><section><h2>Key features</h2>${featureHtml}</section><section><h2>Who it is best for</h2><p>${safeName} is positioned for ${safeAudience}. Confirm that its workflow, integrations, and regional availability match your requirements.</p></section><section><h2>Pricing</h2><p>${escapeHtml(input.pricing || "Confirm current pricing and plan limitations before publishing.")}</p></section><section><h2>Advantages and limitations</h2><div class="generated-columns"><div><h3>Potential advantages</h3><p>Clear value should be confirmed through product research or hands-on evaluation.</p></div><div><h3>Points to verify</h3><p>Check current features, plan restrictions, data handling, support, and cancellation terms.</p></div></div></section><section><h2>Frequently asked questions</h2><h3>Is ${safeName} suitable for beginners?</h3><p>Suitability depends on the workflow and should be tested using a realistic task.</p><h3>How much does it cost?</h3><p>${escapeHtml(input.pricing || "Pricing requires confirmation on the official website.")}</p></section><section class="generated-final"><h2>Final recommendation</h2><p>Consider ${safeName} if it solves a repeatable problem for ${safeAudience}. Verify the marked facts and test the product before a long-term commitment.</p><a class="generated-cta" href="${escapeHtml(input.affiliateUrl)}" rel="sponsored nofollow noopener" target="_blank">Visit ${safeName} ↗</a></section>`;
  return {
    ...
    sections, html, affiliateDisclosure: disclosure,
    warnings: ["Confirm pricing, current features, and regional availability before publishing.", "Replace any placeholder statements with verified facts."],
  };
}
