...
export async function createGeminiDraft(officialUrl: string, affiliateUrl: string, source: { title: string; text: string }) {
  ...
  const prompt = `You are the senior editor for Fikra in Action, an independent practical technology review website. Research only from the supplied official-page text. Create an original, useful, balanced 700-950 word English affiliate landing page. Never invent prices, capabilities, statistics, testimonials, or awards. Mark unavailable facts as needing verification.

The page must feel visual and easy to scan, not like a long essay. Use 1-2 short sentences per paragraph, no paragraph longer than 45 words, and no section longer than 120 words. Prefer cards, bullets, tables, and concise labels over explanatory prose. Remove repetition and marketing filler.

Use separate semantic sections with h2/h3. Include: a very short overview; 4-6 key capabilities as cards; 3-4 use cases as cards; pricing/packages in a table; a short 3-step workflow; pros and cons in two columns; a compact alternatives table; 4-6 FAQs using details/summary; and a brief verdict. Every table must have a thead and tbody, concise cells, and be wrapped exactly in <div class="generated-table-wrap"><table>...</table></div>. Use <div class="generated-card-grid"><article>...</article></div> for feature and use-case cards, <ol class="generated-steps"> for workflow steps, and <div class="generated-pros-cons"> for pros and cons. Do not include h1, scripts, styles, images, iframes, markdown, affiliate disclosure, or a repeated final call to action. Official URL: ${officialUrl}. Affiliate destination: ${affiliateUrl || officialUrl}. Page title observed: ${source.title}. Official page text: ${source.text}`;
  const model = process.env.GEMINI_MODEL || "gemini-3.5-flash";
  const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`;
  const responseSchema = {
    type: "object",
    properties: {
      name: { type: "string" }, category: { type: "string" }, description: { type: "string" }, audience: { type: "string" }, pricing: { type: "string" },
      features: { type: "array", items: { type: "string" } }, title: { type: "string" }, seoTitle: { type: "string" }, seoDescription: { type: "string" },
      keywords: { type: "array", items: { type: "string" } }, articleHtml: { type: "string" },
    },
    required: ["name", "category", "description", "audience", "pricing", "features", "title", "seoTitle", "seoDescription", "keywords", "articleHtml"],
  };
  let response: Response;
  try {
    response = await fetch(endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { responseMimeType: "application/json", responseSchema, temperature: 0.3, maxOutputTokens: 24576 } }), signal: AbortSignal.timeout(55_000), cache: "no-store" });
  } catch {
  ...
  if (!response.ok) {
    ...
    if (response.status === 404) throw new AiGenerationError(`The configured Gemini model (${model}) is unavailable for this API key. Set GEMINI_MODEL to gemini-2.5-flash and redeploy.`, 503);
  ...
  if (finishReason === "MAX_TOKENS") throw new AiGenerationError("Gemini stopped before completing the article. Please try again; the generator now requests a larger output allowance.", 502);
  try {
...
export function assembleAiPage(draft: GeminiDraft, officialUrl: string, affiliateUrl: string, images: GeneratedImage[], videos: Video[]): { page: GeneratedPage; product: GenerationInput } {
  ...
  const imageGallery = images.length > 1 ? `<section class="generated-visual-break"><div><p class="generated-kicker">Visual overview</p><h2>${esc(draft.name)} at a glance</h2><p>Official product visuals and interface previews.</p></div><div class="generated-image-grid">${images.slice(1).map((item) => `<figure class="generated-media"><img src="${esc(item.url)}" alt="${esc(item.alt)}" loading="lazy"/><figcaption>Source: <a href="${esc(item.sourceUrl)}" target="_blank" rel="noopener nofollow">${esc(item.sourceLabel)}</a></figcaption></figure>`).join("")}</div></section>` : "";
  ...
  const html = sanitizeHtml(`<section class="generated-hero">${identity}<p class="generated-kicker">${esc(draft.category)} · Practical guide</p><h1>${esc(draft.title)}</h1><p>${esc(draft.description)}</p>${heroImage}<a class="generated-cta" href="${esc(destination)}" rel="sponsored nofollow noopener" target="_blank">Visit ${esc(draft.name)} ↗</a></section><aside class="generated-disclosure"><strong>Affiliate disclosure:</strong> ${disclosure}</aside>${imageGallery}${draft.articleHtml}${videoHtml}<section class="generated-final"><h2>Explore ${esc(draft.name)}</h2><p>Check the latest plans and terms before subscribing.</p><a class="generated-cta" href="${esc(destination)}" rel="sponsored nofollow noopener" target="_blank">Visit the official website ↗</a></section>`);
